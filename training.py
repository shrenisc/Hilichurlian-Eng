...
def profile_model(params, model):
    model.train()
    dataloader = create_dataloader(params, split = "train")
    progress_bar = tqdm.tqdm(dataloader, desc = "Profiling")
    optimizer = model.config_optimizer()
    scaler = torch.cuda.amp.GradScaler()
    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], 
                 record_shapes=True, with_flops = True, profile_memory = True,
        ...
        for i, data in enumerate(progress_bar):
            ...
            with torch.amp.autocast(device_type = "cuda", dtype = torch.float16):
                loss, _ = model(x = x, article = article, y = y, return_loss = True)
    ...
    prof.export_chrome_trace(params['experiment_path']+f"/trace_experiment{params['experiment_id']}.json")
    with open(f"{params['experiment_path']}"+"/tracing.txt" ,"w+") as txt:
        txt.write(profiler_summary)
        txt.close()
...
def train(params, model):
    ...
    for epoch in range(params['epochs']):
        ...
        for i, data in enumerate(progress_bar):
            ...
            params['writer'].add_scalar("Loss per batch",loss.item(),i)
            params['writer'].add_scalar("Acc per batch",acc.item(),i)
            params['train_loss'].append(loss.item())
            params['train_acc'].append(acc.item())
...
def validation(params, model):
    ...
    params['writer'].add_scalar("Rouge Score",rouge_score,i)
    params['writer'].add_scalar("Bleu Score",bleu_score,i)
    params['bleu_score'].append(bleu_score)
    params['rouge_score'].append(rouge_score)
    model.train()
...
def run_experiment():
    ...
    for i in range(len(experiments.columns)):
        ...
        writer=SummaryWriter(log_dir=metrics_log_path)
